...
//            TDengineDriver.TAOS_MULTI_BIND mbind = TaosMultiBind.MultiBindBool([true]);
//            int bindLengthPtr = Marshal.ReadInt32(mbind.length);
//            bool bindBuffer = Convert.ToBoolean(Marshal.ReadByte(mbind.buffer));


//            Assert.Equal(mbind.buffer_type, bufferType);
//            Assert.Equal(bindBuffer, buffer);
//            Assert.Equal(mbind.buffer_length, bufferLength);
//            Assert.Equal(bindLengthPtr, length);

//            Marshal.FreeHGlobal(mbind.buffer);
//            Marshal.FreeHGlobal(mbind.length);
